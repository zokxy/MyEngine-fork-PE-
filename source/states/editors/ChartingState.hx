...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	public var quantColors:Array<FlxColor> = [
		0xFFDF0000, 0xFF4040CF, 0xFFAF00AF, 0xFFFFAF00, 0xFFFFFFFF, 0xFFFFA0FF,
		0xFFFF6030, 0xFF00CFCF, 0xFF00CF00, 0xFF9F9F9F, 0xFF3F3F3F
	];
	...
	inline function updateVortexColor()
	...
	var bfClick:PsychUICheckBox;
	var opClick:PsychUICheckBox;
	...
	override function create()
	{
		...
		lilStage = new FlxSprite(32, FlxG.height - 256).loadGraphic(Paths.image('editors/lilStage'));
		lilStage.antialiasing = false;
		lilStage.scrollFactor.set();
		add(lilStage);
		
		lilBf = new FlxSprite(32, FlxG.height - 256).loadGraphic(Paths.image('editors/lilBf'), true, 300, 256);
		...
		lilBf.animation.finishCallback = function(name:String) {
		    if (name != "idle") lilBf.animation.play("idle");
		};
		lilBf.antialiasing = false;
		lilBf.scrollFactor.set();
		add(lilBf);

		lilOpp = new FlxSprite(32, FlxG.height - 256).loadGraphic(Paths.image('editors/lilOpp'), true, 300, 256);
		...
		lilOpp.animation.finishCallback = function(name:String) {
		    if (name != "idle") lilOpp.animation.play("idle");
		};
		...
		if(PlayState.SONG == null) openNewChart();
		updateJsonData();
	...
	override function update(elapsed:Float)
	{
				...
				if(backupLimit > 0)
				{
					...
					if(files.length > backupLimit)
					{
						...
						while(files.length > backupLimit)
						{
							...
							try { FileSystem.deleteFile('backups/$file'); } catch(e:Exception) {}
						}
					}
				}
			...
			if(PsychUIInputText.focusOn == null)
			{
				...
				if(vortexEnabled && _keysPressedBuffer.contains(true))
				{
					...
					for (num => press in _keysPressedBuffer)
					{
						...
						for (nIdx in sectionFirstNoteID...notes.length)
						{
							var note = notes[nIdx];
							if(note.strumTime >= strumTime)
							{
								notes.insert(nIdx, noteAdded);
								didAdd = true;
								break;
							}
						}
		...
		if(PsychUIInputText.focusOn == null && lastFocus == null)
		{
				...
				FlxG.keys.justPressed.C || FlxG.keys.justPressed.V || FlxG.keys.justPressed.A || FlxG.keys.justPressed.S))
			{
				...
				else if(FlxG.keys.justPressed.S) saveChart();
			}
			...
			else if(canContinue)
			{
				...
				else if(FlxG.keys.justPressed.Z != FlxG.keys.justPressed.X)
				{
		...
		if(FlxG.mouse.x >= minX && FlxG.mouse.x < gridBg.x + gridBg.width)
		{
			...
			if(isMovingNotes)
			{
				...
				if(movingNotesLastData != nData)
				{
					...
					for (note in movingNotes.members)
					{
						if(note == null || note.isEvent) continue;
				...
				else if(FlxG.mouse.x >= gridBg.x && FlxG.mouse.x < gridBg.x + gridBg.width)
				{
					...
					else if(!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height)
					{
						...
						else if(!lockedEvents)
						{
							...
							for (nIdx in sectionFirstEventID...events.length)
							{
								var event = events[nIdx];
								if(event.strumTime >= strumTime)
								{
									events.insert(nIdx, eventAdded);
									didAdd = true;
									break;
								}
							}
		...
		var checkStepAnim = (curStep != lastStep && FlxG.sound.music != null && FlxG.sound.music.playing && !songFinished);
		
		if(Conductor.songPosition != lastTime || forceDataUpdate || checkStepAnim)
		{
			...
			var bfAnimEnabled = bfClick.checked;
			var oppAnimEnabled = opClick.checked;
			...
			for (note in curRenderedNotes.members)
			{
				...
				if(checkStepAnim && Conductor.songPosition >= note.strumTime && Conductor.songPosition <= note.strumTime + note.sustainLength) {
					if(note.mustPress && bfAnimEnabled) {
						lilBf.animation.play('' + (note.noteData % 4), true);
					} else if(!note.mustPress && oppAnimEnabled) {
				...
				if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
				{
					...
					if(FlxG.sound.music.playing && !songFinished) {
					    if(note.mustPress && bfAnimEnabled) {
			...
			if(checkStepAnim) lastStep = curStep;
		}
	...
	function _cacheSections()
	{
		...
		if(FlxG.sound.music != null && !reachedLimit)
		{
			...
			while(!reachedLimit)
			{
				...
				if(time >= FlxG.sound.music.length) reachedLimit = true;
			}
		}
		cachedSectionRow.push(row);
		cachedSectionTimes.push(time);
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		softReloadNotes();
		updateHeads();
		var sect = getCurChartSection();
		if(sect != null)
		{
			mustHitCheckBox.checked = sect.mustHitSection;
			gfSectionCheckBox.checked = sect.gfSection;
			altAnimSectionCheckBox.checked = sect.altAnim;
			changeBpmCheckBox.checked = sect.changeBPM;
			changeBpmStepper.value = Conductor.bpm;
			beatsPerSecStepper.value = sect.sectionBeats;
		...
		if(!onlyCurrent && (showPreviousSection || showNextSection))
		{
			...
			if(SHOW_EVENT_COLUMN)
			{
				for(event in events)
				{
					...
					if((prevGridBg.visible && event.strumTime >= prevMinTime && event.strumTime < prevMaxTime) ||
					   (nextGridBg.visible && event.strumTime >= nextMinTime && event.strumTime < nextMaxTime))
					{
	...
	inline function getMinNoteTime(sec:Int)
	{
		return (sec > 0) ? cachedSectionTimes[sec] : Math.NEGATIVE_INFINITY;
	}

	inline function getMaxNoteTime(sec:Int)
	{
		return (sec < cachedSectionTimes.length) ? cachedSectionTimes[sec + 1] : Math.POSITIVE_INFINITY;
	}
	...
	function addChartingTab()
	{
		...
		lilBuddiesBox = new PsychUICheckBox(objX, objY + 20, 'Lil\' Buddies', 100, function() {
		...
		bfClick = new PsychUICheckBox(objX + 150, objY + 20, 'BF Note Click', 100);
		opClick = new PsychUICheckBox(objX + 150, objY + 50, 'Opp Note Click', 100);

		tab_group.add(lilBuddiesBox);
		tab_group.add(bfClick);
		tab_group.add(opClick);

		if(chartEditorSave.data.lilBuddies != null) lilBuddiesBox.checked = chartEditorSave.data.lilBuddies;
		if(chartEditorSave.data.bfClick != null) bfClick.checked = chartEditorSave.data.bfClick;
		if(chartEditorSave.data.opClick != null) opClick.checked = chartEditorSave.data.opClick;
	}
	...
	function addDataTab()
	{
		...
		noteTextureInputText.unfocus = function()
		{
			...
			if(PlayState.SONG.arrowSkin.length < 1) PlayState.SONG.arrowSkin = null;
				...
				openSubState(new BasePrompt('Metadata',
					function(state:BasePrompt)
					{
						...
						difficultiesInput.onChange = function(old:String, cur:String)
						{
							...
							while(diffs.contains('')) diffs.remove('');
						}
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Go to...', function()
		{
			...
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
				function(state:BasePrompt)
				{
					...
					var btn2:PsychUIButton = new PsychUIButton(0, btn.y, 'Cancel', state.close);
					btn2.cameras = state.cameras;
					btn2.screenCenter(X);
					btn2.x += 60;
					state.add(btn2);
				}
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Theme...', function()
		{
			...
			openSubState(new BasePrompt(500, 260, 'Chart Editor Theme',
				function(state:BasePrompt)
				{
					...
					var btnY = 320;
					var btn1:PsychUIButton = new PsychUIButton(0, btnY, 'Light', changeTheme.bind(LIGHT));
					btn1.screenCenter(X);
					btn1.x -= 180;
					btn1.cameras = state.cameras;
					state.add(btn1);
			
					var btn2:PsychUIButton = new PsychUIButton(0, btnY, 'Dark', changeTheme.bind(DARK));
					btn2.screenCenter(X);
					btn2.x -= 60;
					btn2.cameras = state.cameras;
					state.add(btn2);
					
					var btn3:PsychUIButton = new PsychUIButton(0, btnY, 'Default', changeTheme.bind(DEFAULT));
					btn3.screenCenter(X);
					btn3.cameras = state.cameras;
					btn3.x += 60;
					state.add(btn3);
			
					var btn4:PsychUIButton = new PsychUIButton(0, btnY, 'V-Slice', changeTheme.bind(VSLICE));
					btn4.screenCenter(X);
					btn4.x += 180;
					btn4.cameras = state.cameras;
					state.add(btn4);

					btnY += 60;
					var btn5:PsychUIButton = new PsychUIButton(0, btnY, 'Custom', changeTheme.bind(CUSTOM));
					btn5.screenCenter(X);
					btn5.x -= 180;
					btn5.cameras = state.cameras;
					state.add(btn5);

					var customBgC:String = '303030';
					if(chartEditorSave.data.customBgColor != null) customBgC = chartEditorSave.data.customBgColor;
					
					var inputBg:PsychUIInputText = new PsychUIInputText(0, btnY, 80, customBgC, 10);
					inputBg.maxLength = 6;
					inputBg.filterMode = ONLY_HEXADECIMAL;
					inputBg.forceCase = UPPER_CASE;
					inputBg.screenCenter(X);
					inputBg.x -= 60;
					inputBg.cameras = state.cameras;
					inputBg.onChange = function(old:String, cur:String)
					{
						chartEditorSave.data.customBgColor = cur;
						changeTheme(CUSTOM);
					}

					var txt:FlxText = new FlxText(inputBg.x, inputBg.y - 15, 120, 'BG Color:');
					...
					var inputGC1:PsychUIInputText = new PsychUIInputText(0, btnY, 80, customGridC[0], 10);
					inputGC1.maxLength = 6;
					inputGC1.filterMode = ONLY_HEXADECIMAL;
					inputGC1.forceCase = UPPER_CASE;
					inputGC1.screenCenter(X);
					inputGC1.x += 60;
					inputGC1.cameras = state.cameras;
					inputGC1.onChange = function(old:String, cur:String)
					{
						chartEditorSave.data.customGridColors[0] = cur;
						changeTheme(CUSTOM);
					}

					var txt2:FlxText = new FlxText(inputGC1.x, inputGC1.y - 15, 120, 'Grid Colors:');
					txt2.cameras = state.cameras;
					state.add(txt2);
					state.add(inputGC1);
					
					var inputGC2:PsychUIInputText = new PsychUIInputText(0, btnY + 30, 80, customGridC[1], 10);
					inputGC2.maxLength = 6;
					inputGC2.filterMode = ONLY_HEXADECIMAL;
					inputGC2.forceCase = UPPER_CASE;
					inputGC2.screenCenter(X);
					inputGC2.x += 60;
					inputGC2.cameras = state.cameras;
					inputGC2.onChange = function(old:String, cur:String)
					{
					...
					var inputGCO1:PsychUIInputText = new PsychUIInputText(0, btnY, 80, customGridOtherC[0], 10);
					inputGCO1.maxLength = 6;
					inputGCO1.filterMode = ONLY_HEXADECIMAL;
					inputGCO1.forceCase = UPPER_CASE;
					inputGCO1.screenCenter(X);
					inputGCO1.x += 180;
					inputGCO1.cameras = state.cameras;
					inputGCO1.onChange = function(old:String, cur:String)
					{
						chartEditorSave.data.customNextGridColors[0] = cur;
						changeTheme(CUSTOM);
					}

					var txt3:FlxText = new FlxText(inputGCO1.x, inputGCO1.y - 15, 120, 'Next Grid Colors:');
					txt3.cameras = state.cameras;
					state.add(txt3);
					state.add(inputGCO1);

					var inputGCO2:PsychUIInputText = new PsychUIInputText(0, btnY + 30, 80, customGridOtherC[1], 10);
					inputGCO2.maxLength = 6;
					inputGCO2.filterMode = ONLY_HEXADECIMAL;
					inputGCO2.forceCase = UPPER_CASE;
					inputGCO2.screenCenter(X);
					inputGCO2.x += 180;
					inputGCO2.cameras = state.cameras;
					inputGCO2.onChange = function(old:String, cur:String)
					{
	...
	inline function getCurChartSection() return PlayState.SONG.notes != null ? PlayState.SONG.notes[curSec] : null;
	...
	function updateUpperBoxBg()
	{
		if(upperBox.selectedTab != null)
		{
			...
			upperBox.bg.setGraphicSize(Std.int(menu.width), Std.int(menu.height + 21));
	...
	function waveformData(buffer:AudioBuffer, bytes:Bytes, time:Float, endTime:Float, multiply:Float = 1, ?array:Array<Array<Array<Float>>>, ?steps:Float):Array<Array<Array<Float>>>
	{
		...
		final multConstant:Float = 1.52590218966964e-5; // 1 / 65535
		
		while (index < (bytes.length - 1)) {
			if (index >= 0) {
				var byte:Int = bytes.getUInt16(index * channels * 2);
				if (byte > 32767) byte -= 65535;

				var sample:Float = (byte * multConstant);
				if (sample > 0)
				{
